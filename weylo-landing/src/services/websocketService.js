...
const REVERB_HOST = import.meta.env.VITE_REVERB_HOST || '10.173.97.19'
  ...
  connect(token, userId) {
    ...
    try {
      this.echo = new Echo({
        ...
        authEndpoint: `${import.meta.env.VITE_API_URL || 'https://10.173.97.19/api/v1'}/broadcasting/auth`,
        auth: {
          headers: {
          ...
          return {
            authorize: (socketId, callback) => {
              ...
              if (channel.name.startsWith('private-') || channel.name.startsWith('presence-')) {
                fetch(`${import.meta.env.VITE_API_URL || 'https://10.173.97.19/api/v1'}/broadcasting/auth`, {
                  method: 'POST',
                  headers: {
                    Authorization: `Bearer ${token}`,
                    Accept: 'application/json',
